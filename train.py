...
def train_loop(model, optimizer, lr_scheduler, loss_fn, tgt_raw_data, dataloader, args, device="cpu"):
    ...
    for idx, batch in enumerate(dataloader):
        ...
        # token_answer = F.one_hot(y_answer[:, :, -1].to(torch.long), num_classes=4)
        # y_answer = torch.concat((y_answer[:, :, :-1], token_answer), dim=2)
        # loss = loss_fn[0](y_pred, y_answer)
        ...
        # logit_to_label = torch.argmax(y_pred[:, :, -4:], dim=2, keepdim=True)
        # y_pred = torch.concat((y_pred[:, :, :-4], logit_to_label), dim=2)
        ...
        # # print(end_token_idx)
        #
        # y_pred_concat = np.concatenate((y_raw[0][0, :-1].reshape(1, y_raw[0][0, :-1].shape[0]),
        #                                 y_pred[0, :end_token_idx, :-1].detach().cpu().data.numpy()), axis=0)
        #
    ...
    return total_loss / len(dataloader), np.mean(
        error_rate), infeasible_cnt / infeasible_test_total_cnt  # np.mean(error_rate) #, np.mean(error_rate_true_end)
    ...
    with torch.no_grad():
        for idx, batch in enumerate(dataloader):
            ...
            if idx == 0:
                ...
                break
    return np.mean(errors_pred), np.mean(errors_sddp), np.var(errors_pred), np.var(errors_sddp), np.var(
        obj_preds), np.var(obj_sddps), np.var(
        obj_msps), pred_cut_ex, encoder_weights, decoder_weights_sa, decoder_weights_mha
...
def predict_one_batch(X, y, idx, model, args, cnt_cuts=2, device="cpu"):
    ...
    for d in range(0, X.shape[0], args.num_stages - 1):
        ...
        # errors_pred.append(get_error_rate(obj_pred, obj_target) / np.abs(obj_target))
        # errors_sddp.append(get_error_rate(obj_target, obj_target) / np.abs(obj_target))
